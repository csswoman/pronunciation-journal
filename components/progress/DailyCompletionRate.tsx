...
export function DailyCompletionRate({ stats }: Props) {
  const hasData = stats.completedDays30 > 0 || stats.activeDays30 > 0 || stats.heatmap30.some((l) => l > 0)
      ...
      {hasData ? (
        <>
          ...
          <div className="mt-2.5 flex items-center justify-between text-body-sm text-fg-muted">
            <span>
              <b className="font-semibold text-fg">{stats.completedDays30}</b> de 30 días con meta
            </span>
            <span>{stats.rate30}% del mes</span>
          </div>

          <div className="mt-2 flex flex-wrap items-center justify-between gap-1.5 border-t border-border-subtle/60 pt-2 text-caption text-fg-muted">
            <span>
        ...
        <div className="flex flex-col items-center gap-1.5 py-4 text-center">
          ...
          <p className="text-caption text-fg-muted max-w-[240px]">
            Cada día que practiques o avances en tu plan se registrará en este mapa de constancia mensual.
          </p>
        </div>
      )}
    </ProgressCard>
