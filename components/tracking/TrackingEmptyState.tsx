...
export function TrackingEmptyState({ filter, onAddWord, onAddPhrase }: Props) {
  ...
  if (filter === "lesson") {
    return <Card className="p-12 text-center"><div className="flex flex-col items-center gap-4"><div className="flex h-16 w-16 items-center justify-center rounded-[var(--radius-lg)] bg-primary-soft text-primary"><BookmarkPlus size={28} aria-hidden /></div><div><p className="text-sm font-semibold text-fg">Aún no guardaste lecciones</p><p className="mt-1 max-w-sm text-sm text-fg-muted">Explora la Ruta y guarda las lecciones a las que quieras volver.</p></div><Link href="/courses" className="inline-flex h-10 items-center gap-2 rounded-[var(--radius-md)] bg-[var(--cta-bg)] px-5 text-sm font-semibold text-[var(--cta-fg)] transition-colors hover:bg-[var(--cta-bg-hover)]"><Plus size={16} aria-hidden />Explorar Ruta</Link></div></Card>;
  }
  return <Card className="p-12 text-center"><div className="flex flex-col items-center gap-4"><div className="flex h-16 w-16 items-center justify-center rounded-[var(--radius-lg)] bg-primary-soft text-primary"><BookmarkPlus size={28} aria-hidden /></div><div><p className="text-sm font-semibold text-fg">{title}</p><p className="mt-1 max-w-sm text-sm text-fg-muted">Guarda contenido que quieras volver a practicar y encuéntralo aquí.</p></div><Button onClick={action} icon={<Plus size={16} />}>{label}</Button></div></Card>;
}
