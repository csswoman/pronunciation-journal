...
export default function HomeWeakPhoneme({
  ...
  return (
    <Card variant="compact" className="gap-4 border-l-2" style={{ background: "var(--surface-raised)", borderLeftColor: "var(--warning)" }}>
      ...
      <h3 className="text-[var(--deep-text)] -mt-1" style={{ font: "var(--font-h3)", fontWeight: 500 }}>Recommended for you</h3>
      ...
      <div className="flex flex-wrap gap-2">
        {PRACTICE_WORDS.map((w) => (
          <Link
            key={w}
            href="/practice"
            className="cursor-pointer text-xs px-[var(--space-3)] min-h-[44px] flex items-center rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-sunken)] text-[var(--text-secondary)] [transition:all_var(--transition-fast,150ms_ease)] hover:bg-[var(--primary-soft)] hover:text-[var(--primary)]"
          >
            {w}
          </Link>
        ))}
