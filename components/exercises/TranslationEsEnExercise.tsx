...
export function TranslationEsEnExercise({ exercise, onResult }: { exercise: Exercise; onResult: (correct: boolean, answer: string, timeMs: number, extras?: GenericRenderExtras) => void }) {
  ...
  return <div className="flex flex-col gap-4"><p className="text-lg text-fg">{exercise.sourceEs}</p><textarea value={answer} onChange={(e) => setAnswer(e.target.value)} rows={3} placeholder="Tradúcelo al inglés…" className="rounded border border-border-default bg-surface-raised p-3 text-fg" />{error ? <p role="alert" className="text-sm text-error">{error}</p> : null}<PillButton variant="primary" size="md" disabled={!answer.trim() || loading} onClick={() => void submit()}>{loading ? 'Corrigiendo…' : 'Comprobar'}</PillButton></div>
}
