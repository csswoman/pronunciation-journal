...
export function TranslationEsEnExercise({ exercise, onResult }: { exercise: Exercise; onResult: (correct: boolean, answer: string, timeMs: number, extras?: GenericRenderExtras) => void }) {
  ...
  return <div className="flex flex-col gap-layout-stack-loose"><p className="max-w-[65ch] text-body-lg leading-relaxed text-fg">{exercise.sourceEs}</p><textarea value={answer} onChange={(e) => setAnswer(e.target.value)} rows={3} placeholder="Tradúcelo al inglés…" className="rounded-md border border-border-default bg-surface-sunken px-4 py-3 text-body-md leading-relaxed text-fg focus-ring" />{error ? <p role="alert" className="text-body-sm text-error">{error}</p> : null}<Button variant="primary" size="lg" fullWidth disabled={!answer.trim() || loading} onClick={() => void submit()}>{loading ? 'Corrigiendo…' : 'Comprobar'}</Button></div>
}
