...
//     ActiveGameCard (Link to /practice/word-rain with icon, description, CTA)
    ...
    <div className="flex flex-col justify-between gap-5 rounded-[var(--radius-lg)] border border-border-default bg-surface-raised p-5 md:p-6 shadow-xs transition-all duration-200 hover:border-border-strong hover:shadow-sm">
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          ...
          {/* Juego Activo 2: Lluvia de palabras */}
          <Link
            href="/practice/word-rain"
            onClick={() => void setLastPracticeMode('word-rain')}
            className="group/game flex flex-col justify-between gap-3.5 rounded-xl border border-border-subtle bg-surface-sunken/60 p-3.5 md:p-4 transition-all duration-200 hover:border-primary/40 hover:bg-surface-sunken active:scale-[0.99] focus-ring"
          >
            <div className="flex items-start gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-border-subtle bg-surface-raised text-primary shadow-2xs group-hover/game:border-primary/30 transition-colors">
              ...
              <div className="flex flex-col gap-0.5 min-w-0">
                ...
                <p className="text-body-xs text-fg-muted">
                  Escribe las palabras antes de que toquen el suelo según tu nivel y guárdalas en tu vocabulario.
                </p>
              </div>
            </div>
      ...
      <div className="flex flex-col gap-2 pt-1 border-t border-border-subtle/60">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {['Word Chain', 'Chunk Duel', 'Phoneme Invaders'].map(
            (gameTitle) => (
              <span
                key={gameTitle}
