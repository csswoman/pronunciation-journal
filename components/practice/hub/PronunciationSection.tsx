...
import { MicVocal, Sparkles, Waves, Layers, ArrowRight } from '@/components/icons'
...
export default function PronunciationSection() {
  ...
  return (
    <PracticeCategoryLane title={category.title} kicker={category.kicker}>
      ...
      <Link href="/practice/minimal-pairs" onClick={() => void setLastPracticeMode('minimal-pairs')} className="flex shrink-0 w-[82vw] max-w-[280px] md:w-auto md:max-w-none snap-start flex-col justify-between gap-3 rounded-[var(--radius-xl)] border border-border-subtle bg-surface-raised p-4 transition-colors hover:bg-surface-sunken focus-ring group">
        ...
        <div className="flex flex-col gap-1"><div className="flex items-center justify-between gap-1"><span className="font-label font-semibold text-fg group-hover:text-primary transition-colors">Pares mínimos</span><ArrowRight size={15} className="text-fg-subtle group-hover:text-primary group-hover:translate-x-0.5 transition-transform" /></div><p className="font-caption text-fg-muted">Entrena la discriminación entre sonidos que se confunden fácilmente</p></div>
      </Link>
