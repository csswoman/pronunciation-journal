// Planned structure:
// <ReferenceSection> — "Diccionario" bento card
//   header: kicker + title + subtitle
//   quick "busca una palabra" affordance
...
export default function ReferenceSection() {
  return (
    <Link
      ...
      className="group relative flex flex-col justify-between gap-5 overflow-hidden rounded-[var(--radius-lg)] border border-border-default bg-surface-raised p-5 md:p-6 shadow-xs transition-all duration-200 hover:border-border-strong hover:shadow-sm active:scale-[0.99] focus-ring"
    >
      ...
      <div className="pt-2 z-10">
        <div className="flex items-center justify-between gap-2 rounded-lg border border-border-subtle bg-surface-sunken/80 px-3 py-2 text-caption text-fg-muted transition-colors group-hover:border-border-default">
          ...
          <ArrowRight
            size={12}
            className="text-fg-subtle transition-transform duration-200 group-hover:translate-x-0.5 group-hover:text-primary"
            aria-hidden="true"
          />
        </div>
      </div>
