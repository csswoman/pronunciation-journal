...
export function FocusSongVoicePractice({ body, onProgress }: { body: SongBody; onProgress: (action: FocusPracticeAction) => void }) {
  const lines = useMemo(() => {
  ...
  useEffect(() => {
    if (status === 'listening' && !startedRef.current) {
  ...
  if (!target) {
    ...
    return <div className="rounded-3xl border border-border-default bg-surface-raised p-6" role="status">
      ...
      <Button className="mt-5" variant="secondary" onClick={() => { setIndex(0); setScores([]); reset() }}>Volver a grabar</Button>
    </div>
  }
  ...
  return <div className="rounded-3xl border border-border-default bg-surface-raised p-6">
    ...
    {current && <div className="mt-5 rounded-xl border border-border-default bg-surface-sunken p-4" role="status">
      ...
      <Button className="mt-4" onClick={() => { onProgress({ kind: 'answered', exerciseId: `voice-${index}` }); if (index + 1 === lines.length) onProgress({ kind: 'completed' }); setScores((previous) => [...previous, current]); setIndex((value) => value + 1) }}>{index + 1 === lines.length ? 'Ver resultado' : 'Siguiente línea'}</Button>
    </div>}
  </div>
}
