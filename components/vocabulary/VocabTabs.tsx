      ...
      {VOCAB_TABS.map(({ id, label, icon: Icon }) => {
        ...
        return (
          <button
            key={id}
            onClick={() => onChange(id)}
            className={`flex items-center gap-1.5 px-4 py-3 text-body-sm -mb-px border-b-2 transition-colors whitespace-nowrap cursor-pointer bg-transparent ${ isActive ? "font-semibold text-fg border-primary" : "font-normal text-fg-muted border-transparent" }`}
          >
