...
import { BATCH_SIZE, PHONEME_TIPS, fetchWordIPA, loadMasteredFromDexie, loadQueueFromDexie, loadSeenFromDexie, pickBatch, saveMasteredToDexie, saveQueueToDexie, saveSeenToDexie, shuffle, speakPhrase } from "@/lib/ai-coach/pronunciation";
...
export default function PronunciationView() {
  ...
  useEffect(() => {
    let cancelled = false;
    void Promise.all([loadMasteredFromDexie(), loadQueueFromDexie(), loadSeenFromDexie()]).then(([storedMastered, storedQueue, storedSeen]) => {
      if (cancelled) return;
