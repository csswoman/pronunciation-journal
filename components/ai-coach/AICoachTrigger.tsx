...
function TriggerIcon({ isOpen }: { isOpen: boolean }) {
  return isOpen ? <CloseIcon /> : <span aria-hidden className="text-body-lg font-bold">✦</span>;
}
...
export default function AICoachTrigger({ variant = "fab", className }: AICoachTriggerProps) {
  ...
  return (
    <button
      ...
      className={cn( "fixed bottom-6 right-6 z-40 w-11 h-11 rounded-2xl flex items-center justify-center text-on-primary text-body-lg font-bold transition-all duration-200 lg:bottom-8 lg:right-8", className, )}
      style={{
