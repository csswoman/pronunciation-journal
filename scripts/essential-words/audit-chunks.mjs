 ...
 *
 * Usage:
 *   node scripts/essential-words/audit-chunks.mjs           # audit all chunks
 *   node scripts/essential-words/audit-chunks.mjs 11 16     # audit range
 *   node scripts/essential-words/audit-chunks.mjs --json    # output JSON report
 */
import fs from "fs";
...
function auditChunk(chunkNum) {
  const file = path.join(ROOT, "public/essential-words", `words-${String(chunkNum).padStart(3, "0")}.json`);
