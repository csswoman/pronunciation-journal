...
export function generateReorderWordsFromWordBank(
  ...
  const usable = entries.filter(e => {
    ...
    if (tokenize(e.example).length < MIN_TOKENS) return false
    // Caps the sentence itself, independent of `entry.difficulty`.
    if (!fitsReorderLength(e.example, learnerLevel)) return false
    return true
  })
