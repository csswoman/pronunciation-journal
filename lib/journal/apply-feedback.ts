...
import { persistJournalCorrection } from './server-queries'
...
export async function applyJournalFeedback(
  ...
  const patterns = correction.errors
    .map((error) => journalErrorToPatternId(error))
    .filter((pattern): pattern is NonNullable<typeof pattern> => pattern !== null)
  const persisted = await persistJournalCorrection(supabase, {
    userId,
    entryId,
    correction,
    feedback,
