// CI gate: public/core-1000/level-index.json must stay in sync with the
...
//
// Regenerate with: node scripts/core-1000/generate-level-index.mjs
import fs from "fs";
...
const INDEX_PATH = path.join(process.cwd(), "public", "core-1000", "level-index.json");
...
describe("Core 1000 level index", () => {
  ...
  it("covers exactly the chunk dataset, in order", () => {
    ...
    expect(
      drift,
      "level-index.json is stale — run `node scripts/core-1000/generate-level-index.mjs`",
    ).toEqual([]);
  });
