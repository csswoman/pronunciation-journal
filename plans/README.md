...
| 071 | Benchmark a free Web-Audio vowel acoustic evaluator | P1 product/pedagogy | L | 064, 067 | IN PROGRESS (2026-07-25: spec + executable plan written; `AcousticEvaluator` widened with `formant_dsp` kind; pure LPC formant-extraction, nearest-centroid vowel classifier, `FormantVowelEvaluator`, Node-side benchmark harness (WAV decoder, metrics, fixed ship threshold ≥0.85, runner) all implemented and unit-tested against synthetic signals — 251/252 focused tests pass (1 pre-existing unrelated failure, see 072 note), typecheck and lint clean. BLOCKED on real corpus run: speechocean762 vowel subset must be downloaded and labeled manually by the human partner (plan Task 6) before `run-benchmark.ts` can produce a real `decision.md` verdict; the conditional gate-release task (Task 11) cannot proceed until that verdict exists. Vowel scoring in 067 remains `not_measured` until then) |
