...
const levels: LessonLevel[] = ["basic", "intermediate", "advanced"];
...
export default function MiniLessonsPage() {
  ...
  const filteredLessons = miniLessons.filter(
    (lesson) =>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-12">
        <Link href="/courses" className="text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] mb-4 inline-block">
          ← Back to courses
        </Link>
        <h1 className="text-4xl font-bold text-[var(--text-primary)] mb-2">Mini Lessons</h1>
        <p className="text-lg text-[var(--text-secondary)]">
          50 focused lessons covering pronunciation, grammar, vocabulary, and more
        </p>
      </div>
      ...
      <p className="text-sm text-[var(--text-tertiary)] mb-6">
        {filteredLessons.length} of {miniLessons.length} lessons
      </p>
